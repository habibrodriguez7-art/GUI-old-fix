...
function Library:CreateMultiDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValues)
    ...
    -- ScrollingFrame for virtual scrolling (TANPA UIListLayout!)
    local listContainer = new("ScrollingFrame", {
        Parent = contentContainer,
        Size = UDim2.new(1, 0, 0, 0),
        Position = UDim2.new(0, 0, 0, 28),
    ...
    -- PENTING: JANGAN tambahkan UIListLayout di sini!
    ...
    local function createItemButton(itemName, index)
        local itemBtn = new("TextButton", {
            ...
            Position = UDim2.new(0, 0, 0, (index - 1) * ITEM_HEIGHT),
            BackgroundColor3 = colors.bg4,
            BackgroundTransparency = 0.4,
            BorderSizePixel = 0,
            Text = "",
    ...
    function updateVisibleItems()
        ...
        local startIdx = math.max(1, math.floor(scrollPos / ITEM_HEIGHT))
        local endIdx = math.min(#filteredItems, startIdx + VISIBLE_ITEMS + 2)
        
        -- Create only visible items with absolute positioning
    ...
    local function updateListDisplay()
        -- Set canvas size based on total filtered items
    ...
    -- Connect scroll event for virtual scrolling
