...
-- ===== DEBOUNCE UTILITY =====
local debounceFlags = {}
local function debounce(key, delay, func)
    if debounceFlags[key] then return end
    debounceFlags[key] = true
    func()
    task.delay(delay, function()
        debounceFlags[key] = false
    end)
end

-- ===== SAVE CONFIG RATE LIMITING =====
local lastSaveTime = 0
local saveDelay = 2 -- Minimum 2 seconds between saves
...
-- Auto-save (respects AutoSaveEnabled flag + rate limiting)
function SaveConfig()
    ...
    -- Rate limiting to prevent save spam
    local now = tick()
    if now - lastSaveTime < saveDelay then
...
local function MakeDraggable(topbarobject, object)
    -- Simple drag without global listeners
    local dragging = false
    local dragStart, startPos
    
    -- Set default size
    if isMobile then
        object.Size = UDim2.new(0, 470, 0, 270)
    else
        object.Size = UDim2.new(0, 640, 0, 400)
    end
...
function Lynx:MakeNotify(NotifyConfig)
    ...
    task.spawn(function()
        ...
        Title.Text = NotifyConfig.Title .. " - " .. NotifyConfig.Description
        ...
        Content.Position = UDim2.new(0, 8, 0, 25)
        ...
        task.delay(tonumber(NotifyConfig.Delay), function()
...
function Lynx:Window(GuiConfig)
    ...
    -- OPTIMIZED: Disable shadow effects on mobile for better performance
    if isMobile then
        DropShadow.ImageTransparency = 1
    end
    ...
    local function UpdateSize1()
        -- OPTIMIZED: Use UIListLayout.AbsoluteContentSize instead of manual loop
        local layout = ScrollTab:FindFirstChildOfClass("UIListLayout")
        if layout then
            ScrollTab.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y + 10)
        end
    end
    -- Register connections for proper cleanup
    AddConnection(ScrollTab.ChildAdded:Connect(UpdateSize1))
    AddConnection(ScrollTab.ChildRemoved:Connect(UpdateSize1))

    function GuiFunc:DestroyGui()
        -- OPTIMIZED: Aggressive memory cleanup for low-end devices
        
        -- Cleanup all connections first
        CleanupAllConnections()
        
        -- Clear all tables to release memory
        table.clear(Elements)
        table.clear(debounceFlags)
        table.clear(ConfigData)
        
        -- Clear text content to reduce memory footprint
        pcall(function()
            for _, obj in pairs(LynxGui:GetDescendants()) do
    ...
    function Tabs:AddTab(TabConfig)
        ...
        TabButton.Activated:Connect(function()
            ...
            if FrameChoose ~= nil and Tab.LayoutOrder ~= LayersPageLayout.CurrentPage.LayoutOrder then
                -- Reset all tabs instantly
                for _, TabFrame in ScrollTab:GetChildren() do
                    if TabFrame.Name == "Tab" then
                        TabFrame.BackgroundTransparency = 0.999
                    end
                end
                -- Set selected tab instantly
                Tab.BackgroundTransparency = 0.92
                FrameChoose.Position = UDim2.new(0, 2, 0, 9 + (33 * Tab.LayoutOrder))
        ...
        function Sections:AddSection(Title, AlwaysOpen)
            ...
            local function UpdateSizeSection()
                if OpenSection then
                    ...
                    -- OPTIMIZED: Use UIListLayout.AbsoluteContentSize instead of manual loop
                    local sectionLayout = SectionAdd:FindFirstChildOfClass("UIListLayout")
                    local SectionSizeYWitdh = 38
                    if sectionLayout then
                        SectionSizeYWitdh = 38 + sectionLayout.AbsoluteContentSize.Y + 3
                    end
                    -- Instant changes (no tween)
            ...
            -- Register connections for proper cleanup
            AddConnection(SectionAdd.ChildAdded:Connect(UpdateSizeSection))
            AddConnection(SectionAdd.ChildRemoved:Connect(UpdateSizeSection))
            ...
            function Items:AddToggle(ToggleConfig)
                ...
                -- OPTIMIZED: Add debounce to prevent TextBounds calculation spam
                AddConnection(ToggleContent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
                    debounce("textsize_toggle_" .. ToggleConfig.Title, 0.15, function()
            ...
            function Items:AddSlider(SliderConfig)
                ...
                -- OPTIMIZED: Add debounce to prevent TextBounds calculation spam
                AddConnection(SliderContent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
                    debounce("textsize_slider_" .. SliderConfig.Title, 0.15, function()
                ...
                -- OPTIMIZED: Fix slider drag connection leak - only connect when dragging
                ...
                function SliderFunc:Set(Value)
                    ...
                    -- Instant size change (no tween)
                    SliderDraggable.Size = UDim2.fromScale((Value - SliderConfig.Min) / (SliderConfig.Max - SliderConfig.Min), 1)
                ...
                AddConnection(SliderFrame.InputBegan:Connect(function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        Dragging = true
                        SliderCircle.Size = UDim2.new(0, 14, 0, 14) -- Instant
                        ...
                        -- Create drag connection only when dragging starts
                        if dragConnection then
                            dragConnection:Disconnect()
                        end
                        dragConnection = AddConnection(UserInputService.InputChanged:Connect(function(inp)
                            if inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch then
                                local scale = math.clamp(
                                    (inp.Position.X - SliderFrame.AbsolutePosition.X) / SliderFrame.AbsoluteSize.X,
                                    0,
                                    1
                                )
                ...
                AddConnection(SliderFrame.InputEnded:Connect(function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        Dragging = false
                        SliderConfig.Callback(SliderFunc.Value)
                        SliderCircle.Size = UDim2.new(0, 8, 0, 8) -- Instant
                        
                        -- Disconnect drag connection when dragging ends
                        if dragConnection then
                            dragConnection:Disconnect()
                            dragConnection = nil
                        end
            ...
            function Items:AddInput(InputConfig)
                ...
                -- OPTIMIZED: Add debounce to prevent TextBounds calculation spam
                AddConnection(InputContent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
                    debounce("textsize_input_" .. InputConfig.Title, 0.15, function()
                ...
                -- OPTIMIZED: Add debounce to prevent FocusLost spam
                AddConnection(InputTextBox.FocusLost:Connect(function()
                    debounce("input_" .. configKey, 0.2, function()
            ...
            function Items:AddDropdown(DropdownConfig)
                ...
                DropdownButton.Activated:Connect(function()
                    if not MoreBlur.Visible then
                        ...
                        DropdownSelect.Position = UDim2.new(1, -11, 0.5, 0)
                    end
                end)
                ...
                AddConnection(UIListLayout4:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                    ScrollSelect.CanvasSize = UDim2.new(0, 0, 0, UIListLayout4.AbsoluteContentSize.Y)
                end))

                -- OPTIMIZED: Add debounce to SearchBox to prevent lag on fast typing
                AddConnection(SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
                    debounce("search_dropdown_" .. DropdownConfig.Title, 0.3, function()
                ...
                -- OPTIMIZED: Lazy loading for large dropdown lists to prevent frame freezes
                function DropdownFunc:SetValues(newList, selecting)
                    newList = newList or {}
                    selecting = selecting or (DropdownConfig.Multi and {} or nil)
                    DropdownFunc:Clear()
                    
                    -- Batch create to avoid frame drops
                    local batchSize = 15
                    local index = 1
                    
                    local function createBatch()
                        local endIndex = math.min(index + batchSize - 1, #newList)
                        for i = index, endIndex do
                            DropdownFunc:AddOption(newList[i])
                        end
                        index = endIndex + 1
