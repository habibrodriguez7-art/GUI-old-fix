...
-- Cached TweenInfo (reused instead of creating new ones each time)
...
local TWEEN_SMOOTH = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
local TWEEN_SLOW = TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
local TWEEN_BACK = TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.InOut)
local TWEEN_BACK_SLOW = TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.InOut)
local TWEEN_BACK_FAST = TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.InOut)
...
-- Debounce helper (prevents spam)
local debounceFlags = {}
local function Debounce(key, callback, delay)
    if debounceFlags[key] then return end
    debounceFlags[key] = true
    task.delay(delay or 0.05, function()
        debounceFlags[key] = nil
        if callback then
            callback()
        end
    end)
...
local function MakeDraggable(topbarobject, object)
    local function CustomPos(topbarobject, object)
        ...
        local function UpdatePos(input)
            ...
            TweenService:Create(object, TWEEN_NORMAL, { Position = pos }):Play()
        end
    ...
    local function CustomSize(object)
        ...
        local function UpdateSize(input)
            ...
            TweenService:Create(object, TWEEN_NORMAL, { Size = UDim2.new(0, newWidth, 0, newHeight) }):Play()
        end
        ...
        if not CoreGui.NotifyGui:FindFirstChild("NotifyLayout") then
            ...
            CoreGui.NotifyGui.NotifyLayout.ChildRemoved:Connect(function()
                ...
                for i, v in CoreGui.NotifyGui.NotifyLayout:GetChildren() do
                    TweenService:Create(v, TWEEN_SMOOTH, 
...
function Lynx:Window(GuiConfig)
    ...
    function GuiFunc:DestroyGui()
        ...
        -- Clear tables to release memory
        table.clear(Elements)
        table.clear(debounceFlags)
        -- Destroy the GUI
    ...
    ConnectButton.Activated:Connect(function()
        if MoreBlur.Visible then
            TweenService:Create(MoreBlur, TWEEN_SMOOTH, { BackgroundTransparency = 0.999 }):Play()
            TweenService:Create(DropdownSelect, TWEEN_SMOOTH, { Position = UDim2.new(1, 172, 0.5, 0) }):Play()
            task.wait(0.3)
            MoreBlur.Visible = false
        end
    end)
            ...
            if FrameChoose ~= nil and Tab.LayoutOrder ~= LayersPageLayout.CurrentPage.LayoutOrder then
                for _, TabFrame in ScrollTab:GetChildren() do
                    if TabFrame.Name == "Tab" then
                        TweenService:Create(TabFrame, TWEEN_BACK, { BackgroundTransparency = 0.999 }):Play()
                    end
                end
                TweenService:Create(Tab, TWEEN_BACK_SLOW, { BackgroundTransparency = 0.92 }):Play()
                TweenService:Create(FrameChoose, TWEEN_SLOW, { Position = UDim2.new(0, 2, 0, 9 + (33 * Tab.LayoutOrder)) }):Play()
                LayersPageLayout:JumpToIndex(Tab.LayoutOrder)
                task.wait(0.05)
                NameTab.Text = TabConfig.Name
                TweenService:Create(FrameChoose, TWEEN_SMOOTH, { Size = UDim2.new(0, 1, 0, 20) }):Play()
                task.wait(0.2)
                TweenService:Create(FrameChoose, TWEEN_NORMAL, { Size = UDim2.new(0, 1, 0, 12) }):Play()
            end
        end)
        --// Section
        local Sections = {}
        ...
        function Sections:AddSection(Title, AlwaysOpen)
            ...
            local function UpdateSizeSection()
                if OpenSection then
                    ...
                    TweenService:Create(FeatureFrame, TWEEN_SLOW, { Rotation = 90 }):Play()
                    TweenService:Create(Section, TWEEN_SLOW, { Size = UDim2.new(1, 1, 0, SectionSizeYWitdh) }):Play()
                    TweenService:Create(SectionAdd, TWEEN_SLOW, { Size = UDim2.new(1, 0, 0, SectionSizeYWitdh - 38) }):Play()
                    TweenService:Create(SectionDecideFrame, TWEEN_SLOW, { Size = UDim2.new(1, 0, 0, 2) }):Play()
                    task.wait(0.5)
                    UpdateSizeScroll()
                else
                    TweenService:Create(FeatureFrame, TWEEN_SLOW, { Rotation = 0 }):Play()
                    TweenService:Create(Section, TWEEN_SLOW, { Size = UDim2.new(1, 0, 0, 30) }):Play()
                    TweenService:Create(SectionDecideFrame, TWEEN_SLOW, { Size = UDim2.new(0, 0, 0, 2) }):Play()
            ...
            function Items:AddToggle(ToggleConfig)
                ...
                function ToggleFunc:Set(Value)
                    if typeof(ToggleConfig.Callback) == "function" then
                        ...
                        if ToggleConfig.Save ~= false then
                        ConfigData[configKey] = Value
                        SaveConfig()
                    end
                    if Value then
                        TweenService:Create(ToggleTitle, TWEEN_NORMAL, { TextColor3 = GuiConfig.Color }):Play()
                        TweenService:Create(ToggleCircle, TWEEN_NORMAL, { Position = UDim2.new(0, 15, 0, 0) }):Play()
                        TweenService:Create(UIStroke8, TWEEN_NORMAL, { Color = GuiConfig.Color, Transparency = 0 }):Play()
                        TweenService:Create(FeatureFrame2, TWEEN_NORMAL, { BackgroundColor3 = GuiConfig.Color, BackgroundTransparency = 0 }):Play()
                    else
                        TweenService:Create(ToggleTitle, TWEEN_NORMAL, { TextColor3 = Color3.fromRGB(230, 230, 230) }):Play()
                        TweenService:Create(ToggleCircle, TWEEN_NORMAL, { Position = UDim2.new(0, 0, 0, 0) }):Play()
                        TweenService:Create(UIStroke8, TWEEN_NORMAL, { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.9 }):Play()
                        TweenService:Create(FeatureFrame2, TWEEN_NORMAL, { BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0.92 }):Play()
                    end
                end
            ...
            function Items:AddSlider(SliderConfig)
                ...
                function SliderFunc:Set(Value)
                    ...
                    TweenService:Create(SliderDraggable, TWEEN_SMOOTH,
                ...
                SliderFrame.InputBegan:Connect(function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        Dragging = true
                        TweenService:Create(SliderCircle, TWEEN_NORMAL, { Size = UDim2.new(0, 14, 0, 14) }):Play()
                ...
                SliderFrame.InputEnded:Connect(function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        ...
                        TweenService:Create(SliderCircle, TWEEN_NORMAL, { Size = UDim2.new(0, 8, 0, 8) }):Play()
                    end
                end)
            ...
            function Items:AddDropdown(DropdownConfig)
                ...
                DropdownButton.Activated:Connect(function()
                    if not MoreBlur.Visible then
                        ...
                        TweenService:Create(MoreBlur, TWEEN_SMOOTH, { BackgroundTransparency = 1 }):Play()
                        TweenService:Create(DropdownSelect, TWEEN_SMOOTH, { Position = UDim2.new(1, -11, 0.5, 0) }):Play()
                    end
                end)
                ...
                function DropdownFunc:Set(Value)
                    ...
                    for _, Drop in ScrollSelect:GetChildren() do
                        if Drop.Name == "Option" and Drop:FindFirstChild("OptionText") then
                            ...
                            if selected then
                                TweenService:Create(Drop.ChooseFrame, TWEEN_NORMAL, { Size = UDim2.new(0, 1, 0, 12) }):Play()
                                TweenService:Create(Drop.ChooseFrame.UIStroke, TWEEN_NORMAL, { Transparency = 0 }):Play()
                                TweenService:Create(Drop, TWEEN_NORMAL, { BackgroundTransparency = 0.935 }):Play()
                                table.insert(texts, Drop.OptionText.Text)
                            else
                                TweenService:Create(Drop.ChooseFrame, TWEEN_FAST, { Size = UDim2.new(0, 0, 0, 0) }):Play()
                                TweenService:Create(Drop.ChooseFrame.UIStroke, TWEEN_FAST, { Transparency = 0.999 }):Play()
                                TweenService:Create(Drop, TWEEN_FAST, { BackgroundTransparency = 0.999 }):Play()
                            end
                        end
                    end
