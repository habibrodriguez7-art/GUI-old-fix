...
function Library:CreateWindow(config)
    ...
    -- Navigation Container (Scrollable, No Visible ScrollBar)
...
function Library:CreatePage(name, title, imageId, order)
    ...
    new("UIListLayout", {Parent = contentContainer, Padding = UDim.new(0, 4), SortOrder = Enum.SortOrder.LayoutOrder})
...
function Library:CreateCategory(parent, title)
    ...
    header.MouseButton1Click:Connect(function()
        ...
        categoryFrame.BackgroundTransparency = isOpen and 0.4 or 0.5
    end)
...
function Library:CreateDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue)
    ...
    -- ScrollingFrame for scrollable list
    ...
    local function createItems(filter)
        ...
        listContainer.Size = UDim2.new(1, 0, 0, math.min(count * 30, 180))
    ...
    header.MouseButton1Click:Connect(function()
        ...
        contentContainer.Visible = isOpen
        arrow.Rotation = isOpen and 180 or 0
        dropdownFrame.BackgroundTransparency = isOpen and 0.5 or 0.6
        if isOpen then
            searchBox.Text = ""
            createItems(nil)
        end
...
function Library:CreateMultiDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValues)
    ...
    -- ScrollingFrame WITHOUT UIListLayout
    local listContainer = new("ScrollingFrame", {
        Parent = contentContainer,
        Size = UDim2.new(1, 0, 0, 180), -- Fixed initial size
        Position = UDim2.new(0, 0, 0, 28),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
    ...
    local MAX_CONTAINER_HEIGHT = 180
    
    -- Debounce
    local searchDebounceThread = nil
    local scrollUpdateThread = nil
    ...
    local function filterItems(filter)
        ...
        if not filter or filter == "" then
            ...
            for _, itemName in ipairs(items) do
                if string.find(itemName:lower(), lowerFilter, 1, true) then
                    table.insert(filteredItems, itemName)
                end
            end
        end
    ...
    local function createItemButton(itemName, index)
        local yPos = (index - 1) * ITEM_HEIGHT
    ...
    function updateVisibleItems()
        ...
        if scrollUpdateThread then
            task.cancel(scrollUpdateThread)
        end
        
        scrollUpdateThread = task.spawn(function()
            clearVisibleItems()
            
            if #filteredItems == 0 then return end
            
            local scrollPos = listContainer.CanvasPosition.Y
            local startIdx = math.max(1, math.floor(scrollPos / ITEM_HEIGHT) + 1)
            local endIdx = math.min(#filteredItems, startIdx + VISIBLE_ITEMS + 1)
            
            -- Create visible items
            for i = startIdx, endIdx do
                local itemName = filteredItems[i]
                if itemName then
                    createItemButton(itemName, i)
                end
            end
        end)
    end
    ...
    local function updateListDisplay()
        if #filteredItems == 0 then
            ...
            listContainer.Size = UDim2.new(1, 0, 0, 50)
            contentContainer.Size = UDim2.new(1, -12, 0, 82)
            return
        end
        
        -- Set canvas size
        local totalHeight = #filteredItems * ITEM_HEIGHT
        listContainer.CanvasSize = UDim2.new(0, 0, 0, totalHeight)
        
        -- Set container size
        local containerHeight = math.min(totalHeight, MAX_CONTAINER_HEIGHT)
        listContainer.Size = UDim2.new(1, 0, 0, containerHeight)
        contentContainer.Size = UDim2.new(1, -12, 0, containerHeight + 32)
        
        -- Render initial items
        updateVisibleItems()
    end
    
    local function performSearch(filter)
    ...
    -- Scroll handler
    listContainer:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
        if isOpen then
            updateVisibleItems()
        end
    end)
    ...
    searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        searchDebounceThread = task.delay(0.2, function()
    ...
    if configPath then
        -- Don't register standard callback
    end
